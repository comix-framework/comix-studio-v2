<template>
  ...
  >
    <a class='absolute top-0 left-0 w-full h-full close-overlay cursor-pointer' title='Close' @click='_useModal.dispose()'></a>
    <div
        :id='`${event}Body`'
      ...
      '
    >
      <a class='-translate-x-7 z-10 absolute close-button right-0 text-xl top-0 transform translate-y-5 cursor-pointer' @click='_useModal.dispose()'>
      ...
      <div class='px-7'>
        ...
        >
          <slot :show='_useModal.open.value' :data="_useModal.data.value"></slot>
        </div>
      ...
      <div v-if="$slots['bottom']" class="px-7 relative">
        ...
        <div class="w-full h-px bg-gray-200 mt-3.5 mb-3"></div>
